;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;Code by Wade (a1704756) has done/changed;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
to setup
  clear-all
  create-birds bird-population
    [ set color yellow - 2 + random 7  ;; random shades
      set size 0.5  ;; easier to see
      setxy random-xcor random-ycor
      set flockmates no-turtles
      set nearest-hawk nobody
      set speed bird-speed
  ]
  create-hawks hawk-population [
    set color red - 2 + random 7
    set size 1.5  ;; bigger than the bird
    setxy random-xcor random-ycor
    set speed hawk-speed
  ]
  reset-ticks
end
...
;; Movement
;;Birds Flocking Movement
to flock  ;; turtle procedure

  ;; finds the nearby birds to flock with
  find-flockmates

  ;; avoid the hawk
  avoid-hawk

  ;;if birds have flockmates, turn towards and align. if they are too close seperate a bit
  if any? flockmates
...
;; birds to avoid the hawk
to avoid-hawk
  ;; finds nearby hawks
  find-hawks

  ;; if there is a hawk nearby
  if nearest-hawk != nobody
  [
    ;; turn away from the hawk
    turn-away-from-hawk

    ;; move towards the center of your flock
    head-towrds-center-flockmates

    ;; if hawk is ontop of bird, bird dies
    kill-birds
  ]
end

;; make birds find hawks in its vision
to find-hawks
  let nearest-hawks (hawks) in-radius birds-see-hawk-distance    ;gets all hawks near me
  set nearest-hawk min-one-of (nearest-hawks) [distance myself]  ;gets the closest hawk to me
end

;; turns away from the hawk at a max turn angle
to turn-away-from-hawk
  let direction (towards nearest-hawk)
  turn-away (direction) turn-away-hawk
end

;; make birds head towards center of flock
to head-towrds-center-flockmates
  if any? flockmates
  [
    turn-towards average-flockmate-position max-align-turn
  ]
end

;; kills bird if hakw is too close
to kill-birds
  ;;if hawk is near bird and hawk can kill, then kill bird
  if distance nearest-hawk < 1 and hawk-kills = true
  [
     die
  ]
end

;;Hawks Hunting Movement
to hunt
  ;finds the nearest bird to hunt
  find-current-prey

  ;;if their is a bird in the hawks vision
  if current-prey != nobody
  [
    ;turn towards that bird
    turn-towards ([heading] of current-prey) max-hawk-turn
  ]
end

;; Find current prey in the vision of the hawk
to find-current-prey
  ;;find the set of birds in the hawks vision
  set nearest-birds birds in-radius Hawk-vision-distance

  ;;set the prey to the closest one
  set current-prey min-one-of nearest-birds [distance myself]

  ;;a chance in loosing interest in the nearby bird
  loose-intrest-in-bird
end

to loose-intrest-in-bird
  ;;losses intreast based on random number generator
  if random-float 1 > (loose-intrest / 100) and (count nearest-birds) >= 2
  [
    ;;pick the next closest bird
    let two-nearest-birds (min-n-of 2 nearest-birds [distance myself])
    set current-prey one-of nearest-birds
  ]
end

;;returns the average flockmate position
to-report average-flockmate-position
  let center-flock-x mean [xcor] of flockmates
  let center-flock-y mean [ycor] of flockmates
  report atan center-flock-x center-flock-y
end


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;Unedited code from Libary "Flocking";;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;Birds to find flock mates
to find-flockmates  ;; turtle procedure
  set flockmates other birds in-radius bird-see-bird-distance
end

;;finds the nearest neibouring birds
to find-nearest-neighbor ;; turtle procedure
  set nearest-neighbor min-one-of flockmates [distance myself]
end


;;; go away from nearest birds (helps keep their distance away from each other)
to separate  ;; turtle procedure
  turn-away ([heading] of nearest-neighbor) max-separate-turn
end

;;; Aligns all birds to go in the same direction
to align  ;; turtle procedure
  turn-towards average-flockmate-heading max-align-turn
end

;;returns the average flockmate heading direction
...
to-report number-turtles-flocking
  let number 0
  let birds_flocking any? ( birds with [nearest-neighbor > 5])

  report birds_flocking
end
...
627
843
Number Birds Flocking vs. Ticks
Ticks
Birds Flocking
0.0
100.0
...
"" ""
PENS
"pen-1" 1.0 0 -7500403 true "" "plot count birds with [count flockmates > 20]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot bird-population - (count birds) "
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [count flockmates] of birds"
...
"" ""
PENS
"Number of Flocks" 1.0 0 -13345367 true "" "plot count birds with [count flockmates > 20]"
"Mean FlockSize" 1.0 0 -13840069 true "" "plot mean [count flockmates] of birds"
"Dead Birds" 1.0 0 -2674135 true "" "plot bird-population - (count birds) "
